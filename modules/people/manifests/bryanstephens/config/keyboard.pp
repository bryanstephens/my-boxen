class people::bryanstephens::config::keyboard {
  ...
  karabiner::private_xml { "private.xml":
    source  => "${people::bryanstephens::config::dotfiles_dir}/osx/key_remap.xml",
  ...
  define private_mapping ($setting = $title, $ensure = "enabled") {
    ...
    exec { "people::bryanstephens::config::keyboard::${action}_${identifier}":
      command   => $cmd,
      unless    => $unless_cmd,
      onlyif    => $onlyif_cmd,
      require   => [
